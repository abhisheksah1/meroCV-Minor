...
function Users() {
  ...
  // Function to fetch all users on component mount
  useEffect(() => {
    axios
      .get(`http://localhost:8000/api/adminUser/getAllUsers`)
      .then((response) => {
      ...
      .catch((error) => {
        console.log("Error fetching all users data:", error.message);
      });
  }, []);
  ...
  const handleDelete = (userId) => {
    axios
      .delete(`http://localhost:8000/api/adminUser/deleteUser/${userId}`)
      .then((response) => {
    ...
    <div>
      ...
      <main className="flex-1 p-6 bg-gray-100">
        ...
        <div className="overflow-x-auto">
            ...
            <tbody>
              {filteredUsers.map((user, index) => (
                <tr key={user._id}>
                  ...
                  <td className="px-6 py-4 border-b border-gray-200">{user._id}</td>
                  <td className="px-6 py-4 border-b border-gray-200">{user.fullName}</td>
          ...
          {filteredUsers.length === 0 && <div className="text-center py-4">No user found.</div>}
        </div>
      </main>
    </div>
  );
