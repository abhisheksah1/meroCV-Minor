...
function Subscribe() {
    ...
    useEffect(() => {
      axios
        .get(`http://localhost:8000/api/adminUser/getAllSubscribe`)
        .then((response) => {
          if (response.data && Array.isArray(response.data)) {
            setUsers(response.data);
            console.log("All subscriber data fetched:", response.data);
          } else {
            console.log("No subscriber found.");
          }
        })
        .catch((error) => {
          console.log("Error fetching all subscriber data:", error.message);
        });
    }, []);
  
    const handleDelete = (userId) => {
      axios
        .delete(`http://localhost:8000/api/adminUser/deleteSubscribe/${userId}`)
        .then((response) => {
